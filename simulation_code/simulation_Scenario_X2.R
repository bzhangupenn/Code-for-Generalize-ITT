...
D_md2 <- function(Z, X){
  p <- expit(Z*(2.5 - 0.1 * X[,1] + 0.3 * X[,2] - 0.4 * X[,5]) +
...
# D model for D_h1
D_md1 <- function(Z, X){
  p <- expit(Z*(2 + 0.2*X[,1] - 0.2*X[,5]) + (1-Z)*(- 0.2*X[,5] - 1))
  model <- rbinom(length(Z), 1, prob = p)
  return(list(model = model, p = p) )
}
...
run_simu_once <- function(n_1, n_2, n, c, D_md1, D_md2, Y_md1, Y_md2, Y_md3, 
  ...
  # data generation
  dt_h1 = generate_data_binary(n = n_1, c = 1.2*c, D_md1, Y_md1)
  dt_h2 = generate_data_binary(n = n_2, c = c, D_md2, Y_md2)
  dt = generate_data_binary(n = n, c = 0.8*c, D_md1, Y_md3)
